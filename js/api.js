                ...
                konselor.forEach((k, index) => {
                    ...
                    if (ctx) {
                        new Chart(ctx.getContext('2d'), {
                            type: 'bar',
                            data: { labels: ['S1', 'S2', 'S3', 'S4', 'Kini'], datasets: [{ data: [0, 0, 0, 0, k.rating_rata], backgroundColor: ['#F1F5F9', '#F1F5F9', '#F1F5F9', '#F1F5F9', '#0F172A'], borderRadius: 4 }] },
...
async function initPilihKonselorPage() {
    ...
    async function loadDaftarKonselor() {
        ...
        grid.innerHTML = list.length > 0 ? list.map(k => {
            ...
            const sp = Array.isArray(k.spesialisasi) ? k.spesialisasi : [];
            return `
...
async function initPenjadwalanPage() {
    ...
    dateButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const day = btn.getAttribute('data-date'), now = new Date();
...
async function initLiveChatPage() {
    ...
    async function kirimPesan() {
        const isi = inputChat.value.trim(); if (!isi) return;
        inputChat.value = '';
        await supabaseClient.from('pesan').insert([{ sesi_id: sesiId, pengirim_id: profile.id, isi }]);
        loadPesan();
    }
...
async function initRingkasanSesiPage() {
    ...
    const { data: sesi } = await supabaseClient.from('sesi').select('*, konselor:konselor_id(users:user_id(nama_lengkap, avatar_seed))').eq('id', sesiId).single();
    if (!sesi) return;
...
async function initRiwayatPage() {
    ...
    if (daftarSesi && daftarSesi.length > 0) {
        ...
        const sesiBulanIni = daftarSesi.filter(s => new Date(s.tanggal).getMonth() === bulanIni && s.status === 'Selesai');
        ...
        container.innerHTML = '<div class="col-span-3 text-center py-10 text-gray-400">Belum ada riwayat sesi.</div>';
    }
}
...
window.initDashboardPage = async () => {
    ...
    document.querySelectorAll('.filter-select, input[type="date"]').forEach(el => {
        el.addEventListener('change', () => updateDashboard(rawData));
    });
...
function updateDashboard(data) {
    ...
    const end = new Date(document.getElementById('filterTanggalEnd')?.value || '2026-12-31');
    ...
    if(total > 0) {
        ...
        setText('#statMean', '0.00'); setText('#rekPct', '0%');
    }
}
